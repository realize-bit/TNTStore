...
static void init_dbbench(struct workload *w, bench_t b) {
   switch(b) {
      ...
      case dbbench_prefix_dist:
         gen_exp = init_exp_prefix(w->nb_items_in_db, 30, 14.18, -2.917, 0.0164, -0.08082);
         init_rand();
         return;
...
/* YCSB A (or D), B, C */
static void _launch_dbbench(int total, int nb_requests, double dist_a, double dist_b,
                    int range_num, double range_a, double range_b, double range_c, double range_d) {
   ...
   for(size_t i = 0; i < nb_requests; i++) {
      ...
      //}
      kv_read_async(cb);
      periodic_count(1000, "DB_BENCH Load Injector (%lu%%)", i*100LU/nb_requests);
   }
}
...
static void launch_dbbench(struct workload *w, bench_t b) {
   switch(b) {
      ...
      case dbbench_prefix_random:
         return _launch_dbbench(w->nb_items_in_db, w->nb_requests_per_thread,
                0, 0, 30, 14.18, -2.917, 0.0164, -0.08082);
      case dbbench_prefix_dist:
         return _launch_dbbench(w->nb_items_in_db, w->nb_requests_per_thread,
                0.002312, 0.3467, 30, 14.18, -2.917, 0.0164, -0.08082);
      default:
...
static const char* api_name_dbbench(void) {
   return "DBBENCH";
}
