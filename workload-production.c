  ...
  for (size_t i = 0; i < nb_requests; i++) {
    ...
    /*else {
     ...
     for(size_t j = 0; j < scan_res.nb_entries; j++) {
        cb = bench_cb();
        cb->item = create_unique_item_prod(scan_res.hashes[j],
  w->nb_items_in_db); kv_read_async_no_lookup(cb, scan_res.entries[j].slab,
  scan_res.entries[j].slab_idx, 0);
     }
...
static const char *api_name_prod(void) { return "PRODUCTION"; }
