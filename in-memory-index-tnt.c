...
tree_entry_t *centree_worker_lookup(int worker_id, void *item) {
   return centree_lookup(centree_root, (void*)get_prefix_for_item(item), tnt_pointer_cmp);
}
...
//    W_LOCK(&centree_root_lock);
//    centree_delete(centree_root, (void*)get_prefix_for_item(item), tnt_pointer_cmp);
//    W_UNLOCK(&centree_root_lock);
// }
//
index_entry_t *subtree_worker_lookup_utree(subtree_t *tree, void *item) {
   ...
   int res = subtree_find(tree, (unsigned char*)&hash, sizeof(hash), &tmp_entry);
   if(res)
      return &tmp_entry;
   else
      return NULL;
