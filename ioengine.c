...
/*
 * After completing IOs we need to call all the "linked callbacks", i.e., reads
 * done to a page that was already in the process of being fetched.
 */
  ...
  //start_debug_timer {
    ...
    while (linked_cb) {
      ...
      if (callback->lru_entry->contains_data) {
        ...
        free(linked_cb);
      } else {  // page has not been prefetched yet, it's likely in the list of
                // pages that will be read during the next kernel call
...
static void worker_do_io(struct io_context *ctx) {
  ...
  for (size_t i = 0; i < pending; i++) {
    ...
    callback->lru_entry->dirty =
        0;  // reset the dirty flag *before* sending write orders otherwise
            // following writes might be ignored race condition if flag is reset
            // after:
            //        io_submit
            //        flush done to disk
            //              |                           write page (no IO order
            //              because dirty = 1, see write_page_async
            //              "if(lru_entry->dirty)" condition)
            //        complete ios
            //        (old value written to disk)

    add_time_in_payload(callback, 5);
  }

  // Submit requests to the kernel
  ret = io_submit(ctx->ctx, pending, ctx->iocbs);
  // while (ret < pending) {
  //    printf("Couldn't submit all io requests! %d submitted / %lu (%lu sent,
...
char *no_read_page_async(struct slab_callback *callback) {
  ...
  alread_used =
      get_page(get_pagecache(callback->ctx), hash, &disk_page, &lru_entry);
...
/* Enqueue a request to write a page, the lru entry must contain the content of
 * the page (obviously) */
...
struct io_context *worker_ioengine_init(size_t nb_callbacks) {
  ...
  if (ret < 0) perr("Cannot create aio setup\n");
