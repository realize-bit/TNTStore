...
int btree_find(btree_t *t, unsigned char *k, size_t len,
...
void btree_insert(btree_t *t, unsigned char *k, size_t len,
                  struct index_entry *e) {
  uint64_t hash = *(uint64_t *)k;
  btree_map<uint64_t, struct index_entry> *b =
      static_cast<btree_map<uint64_t, struct index_entry> *>(t);
...
struct index_scan btree_find_n(btree_t *t, unsigned char *k, size_t len,
                               size_t n) {
...
void btree_forall_keys(btree_t *t, void (*cb)(uint64_t h, void *data),
                       void *data) {
  btree_map<uint64_t, struct index_entry> *b =
      static_cast<btree_map<uint64_t, struct index_entry> *>(t);
  auto i = b->begin();
