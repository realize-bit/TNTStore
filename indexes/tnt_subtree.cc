...
extern "C"
{
  ...
  static inline unsigned char test_inval(size_t *addr)
  {
   ...
   int subtree_find(subtree_t *t, unsigned char* k, size_t len, struct index_entry *e) {
   ...
   void subtree_insert(subtree_t *t, unsigned char*k, size_t len, struct index_entry *e) {
      uint64_t hash = *(uint64_t*)k;
      btree_map<uint64_t, struct index_entry> *b = static_cast< btree_map<uint64_t, struct index_entry> * >(t);
      b->insert(make_pair(hash, *e));
   }

   struct index_scan subtree_find_n(subtree_t *t, unsigned char* k, size_t len, size_t n) {
   ...
   void subtree_forall_keys(subtree_t *t, void (*cb)(uint64_t h, void *data), void *data) {
      btree_map<uint64_t, struct index_entry> *b = static_cast< btree_map<uint64_t, struct index_entry> * >(t);
      auto i = b->begin();
      while(i != b->end()) {
         cb(i->first, data);
   ...
   uint64_t subtree_next_key(subtree_t *t,  unsigned char* k, struct index_entry *e) {
      btree_map<uint64_t, struct index_entry> *b = static_cast< btree_map<uint64_t, struct index_entry> * >(t);
      uint64_t hash;
      auto i = b->begin();
