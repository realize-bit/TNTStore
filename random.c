...
long zipf_next() { return next_long(items); }

/* Uniform */
long uniform_next() { return rand_r(&seed) % items; }

/* bogus rand */
long bogus_rand() { return rand_r(&seed) % 1000; }
...
const char *get_function_name(random_gen_t f) {
  if (f == zipf_next) return "Zipf";
  if (f == uniform_next) return "Uniform";
  if (f == bogus_rand) return "Cached";
  if (f == production_random1) return "Production1";
  if (f == production_random2) return "Production2";
