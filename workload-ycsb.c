 ...
 */
  ...
  for (size_t i = 0; i < nb_requests; i++) {
    if (random_get_put(
      ...
      //  printf("(%d, %d, %lu, %lu) ", i, scan_res.entries[i].slab->fd,
      //  scan_res.entries[i].slab_idx, scan_res.hashes[i]);
      // }
      qsort(scan_res.entries, scan_res.nb_entries, sizeof(struct index_entry),
            compare);
      // printf("AFTER: %ld\n", scan_res.nb_entries);
      // for(int i=0; i < scan_res.nb_entries; i++) {
      //  printf("(%d, %d, %lu) ", i, scan_res.entries[i].slab->fd,
      //  scan_res.entries[i].slab_idx);
      //
      // }
      // printf("\n");
      ...
      for (size_t j = 0; j < scan_res.nb_entries;) {
        ...
        */
        kv_read_async_no_lookup(cb, scan_res.entries[j].slab,
                                scan_res.entries[j].slab_idx, count);
        j += count;
      }
      free(scan_res.hashes);
      free(scan_res.entries);
