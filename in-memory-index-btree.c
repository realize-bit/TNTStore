...
index_entry_t *btree_worker_lookup(int worker_id, void *item) {
  ...
  int res = btree_find(items_locations[worker_id], (unsigned char *)&hash,
                       sizeof(hash), &tmp_entry);
  if (res)
    return &tmp_entry;
  else
    return NULL;
...
void btree_worker_insert(int worker_id, void *item, index_entry_t *e) {
  ...
  btree_insert(items_locations[worker_id], (unsigned char *)&hash, sizeof(hash),
               e);
...
void btree_worker_delete(int worker_id, void *item) {
  ...
  pthread_spin_lock(&items_location_locks[worker_id]);
  btree_delete(items_locations[worker_id], (unsigned char *)&(hash),
               sizeof(hash));
  pthread_spin_unlock(&items_location_locks[worker_id]);
...
struct index_scan btree_init_scan(void *item, size_t scan_size) {
  ...
  for (size_t w = 0; w < nb_workers; w++) {
    ...
    res[w] = btree_find_n(items_locations[w], (unsigned char *)&(hash),
...
/*struct index_scan btree_init_scan(void *item, size_t scan_size) {
   ...
   for(size_t w = 0; w < nb_workers; w++) {
      pthread_spin_lock(&items_location_locks[w]);
      res[w] = btree_find_n(items_locations[w], (unsigned char *)&(hash),
sizeof(hash), scan_size); pthread_spin_unlock(&items_location_locks[w]);
   }

   btree_t *b = btree_create();
   for(size_t w = 0; w < nb_workers; w++) {
      for(size_t i = 0; i < res[w].nb_entries; i++) {
         btree_insert(b, (unsigned char*)&(res[w].hashes[i]),
sizeof(res[w].hashes[i]), &res[w].entries[i]);
      }
   }
   scan_res = btree_find_n(b, (unsigned char *)&(hash), sizeof(hash),
scan_size); for(size_t w = 0; w < nb_workers; w++) { free(res[w].hashes);
      free(res[w].entries);
   }
   free(res);
   btree_free(b);
