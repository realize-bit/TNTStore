...
extern "C"
{
  ...
  class GenerateTwoTermExpKeys {
    public:
      ...
      // KeyrangeUnit.
      int InitiateExpDistribution(int64_t total_keys, int range_num, double prefix_a,
  ...
  void init_rand(void) {
    my_rand = new Random64(std::chrono::system_clock::now().time_since_epoch().count());
  }
...
void *init_exp_prefix(int64_t total_keys, int range_num,
  ...
  int64_t prefix_get_key(void *gen_exp, int64_t ini_rand, 
      double key_dist_a, double key_dist_b) {
     return static_cast<GenerateTwoTermExpKeys *>(gen_exp)->DistGetKeyID(ini_rand, key_dist_a,  key_dist_b);
  }
